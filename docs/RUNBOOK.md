...
8. Luna Chat 本体コードは AI 自己改善で更新しない。
...
3. `CODEX_WORKSPACE_DIR` が存在し、書き込み可能であることを確認する。
4. `APOLOGY_TEMPLATE_PATH` が存在することを確認する。
...
2. 文脈取得は Discord API から都度行う。
3. AI 結果に従って返信する。
4. AI エラー時は謝罪定型文で返信する。
...
4. 文脈メッセージ群
5. 謝罪定型文の参照先
6. 自己改善はドキュメント限定という制約

## 5. 失敗時対応

1. Discord 投稿失敗:
   - 失敗をログ化し、再送は 1 回のみ試行する。
2. AI 呼び出し失敗:
   - 謝罪定型文を返信する。
3. 設定不備:
   - 起動を中断する。
