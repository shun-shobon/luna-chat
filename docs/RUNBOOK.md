...
8. ワークスペース（`$LUNA_HOME/workspace`）のドキュメントを AI instructions に読み込む。
9. 自己改善ドキュメントの自動更新フローは現時点で未実装。
10. 秘密情報をログやドキュメントに平文出力しない。
11. `STATUS.md` は作業ごとに AI が更新する。
12. heartbeat は毎時 00 分 / 30 分（JST）に自動実行する。
13. heartbeat 実行時は以下の固定プロンプトを使う。  
...
3. AI は必要時に `read_message_history` / `send_message` / `add_reaction` を使用する。
