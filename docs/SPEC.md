...
Umbra は、身内向け Discord サーバーで雑談に自然参加する Bot を作るプロジェクトである。  
...
- 自己改善はドキュメント更新のみ許可する。
- コード改変は自己改善対象外とする。
- Umbra 本体コードのディレクトリとは別に、Codex CLI 用ワークスペースディレクトリを持つ。
...
6. AI 失敗時に謝罪定型文を返す。
7. 自己改善で更新されるのは Codex ワークスペース内ドキュメントのみで、Umbra 本体コードは更新しない。
