...
- プロジェクトは MVP 定義を完了した。
- Luna Chat 本体は最小実装で、現状は `!ping` 応答中心の段階。
...
1. `!ping` 中心の現行実装を、チャンネル限定 + メンション必須返信フローへ置換する。
2. Discord 都度履歴取得ロジックを実装する。
3. Codex CLI app-server 呼び出し層を実装する。
4. 謝罪定型文を Codex ワークスペース管理へ移す。
5. ユニット / 結合テストを追加する。
